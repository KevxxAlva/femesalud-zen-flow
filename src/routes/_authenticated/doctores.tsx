...
export const Route = createFileRoute("/_authenticated/doctores")({
  head: () => ({ meta: [{ title: "Doctores — Medizen" }] }),
  beforeLoad: async () => {
    ...
    <div className="bg-card rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-6 shadow-sm min-h-[calc(100vh-8rem)] font-sans flex flex-col">
      ...
      <header className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-4 hidden md:flex">
          <button onClick={() => setOpenCreate(true)} className="flex items-center gap-2 bg-primary text-primary-foreground px-5 py-2.5 rounded-xl text-xs font-bold shadow-md hover:bg-[#3451d6] transition">
      ...
      {/* SECONDARY TOOLBAR */}
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4 border-b border-border/40 pb-4 mt-2">
        <div className="flex items-center gap-2 text-sm font-bold text-muted-foreground">
          <Users className="h-4 w-4" /> {filtered.length} miembros
        </div>
        <div className="flex items-center gap-3 md:hidden">
          <button onClick={() => setOpenCreate(true)} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-xl text-xs font-bold shadow-md hover:bg-[#3451d6] transition">
            <Plus className="h-3.5 w-3.5" /> Añadir Personal
          </button>
        </div>
      </div>
